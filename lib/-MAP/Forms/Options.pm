...
our $defaultColumns = 'page_id,option_id,field_id,type,type_standard,name,label,asdefault,caption,tooltip,text_size,required,className,mask_to_use,value,info,note,index,FieldOptionSeq,optionname,text';
...
post '/'.$collectionName.'.:format' => sub {
	...
	my $strSQLtable = '
		...
		BEGIN
			ALTER TABLE dbo.formmaker_'.$agency_id .'_'.$formname.'_tmp ADD '.$hash{name}.' varchar(MAX);
		END
	';
...
put '/'.$collectionName.'/:'.$primaryKey.'.:format' => sub {
	...
	for(@IDs)
	{
		...
		my $field_name = undef;
		$strSQLquery = 'SELECT opWtion_id, name FROM ' . $tableName . ' WHERE ['.$primaryKey.'] = ' . $_;
		...
		my $strSQLtable = '
			...
			BEGIN
				ALTER TABLE dbo.formmaker_'.$agency_id .'_'.$formname.'_tmp ADD '.$field_name.' varchar(MAX);
			END
		';
		$sth = $dbh->prepare( $strSQLtable, );
		$sth->execute(  ) or MAP::API->fail( $sth->errstr . " --------- ".$strSQLtable );
