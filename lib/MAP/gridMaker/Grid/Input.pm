package MAP::gridMaker::grid::Input;
use Dancer ':syntax';
...
our $VERSION = '0.1';
my $collectionName = 'records';
my $primaryKey = 'data_id';
my $tableName = '';
my $defaultColumns = '';
...
prefix $prefix; # | undef
 ...
 # routing OPTIONS header
	  ...
	  get '/'.$collectionName.'/doc' => sub {
		 ...
		 $tableName =  params->{table_name} || MAP::API->fail( 'the param table_name is missing' );
	  ...
	  post '/'.$collectionName.'.:format' => sub {
		 ...
		 $defaultColumns = $defaultColumns . $primaryKey;
		  ...
		  my $strSQL = 'INSERT INTO '.$tableName.'(' . substr($sql_columns, 0, -2) . ') VALUES(' . substr($sql_placeholders, 0, -2) . ');
