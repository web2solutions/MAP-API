...
my $primaryKey = 'address_id';
my $tableName = 'tbl_clients_adress_demo';
my $defaultColumns = 'address_id,client_id,address_type,address,city,zip,state,country';

my $relationalColumn = 'client_id'; # undef
# ======== CHANGE HERE

prefix '/client/:'. $relationalColumn; # | undef
...
# HELPERS
#/forms/2975/pages/order.json
# update item on collection
post '/'.$collectionName.'/order.:format' => sub {
	...
	for(@$data)
	{
		...
		my $sth = $dbh->prepare( $strSQL, );
		$sth->execute( $index, $item_id ) or MAP::API->fail( $sth->errstr ); 
	}
